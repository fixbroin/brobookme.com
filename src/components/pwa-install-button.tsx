...
export default function PwaInstallButton({ appName, appDesc, appIcon }: PwaInstallButtonProps) {
  ...
  // Determine which app we are installing if props aren't provided
  const resolvedAppName = appName || (
    pathname.startsWith('/admin') ? "BBM Admin" :
    (pathname.startsWith('/dashboard') || pathname.startsWith('/profile') || pathname.startsWith('/bookings') || pathname.startsWith('/services')) ? "BBM Provider" :
    "BroBookMe"
  );

  const resolvedAppDesc = appDesc || (
    pathname.startsWith('/admin') ? "Manage orders & providers" :
    (pathname.startsWith('/dashboard') || pathname.startsWith('/profile') || pathname.startsWith('/bookings') || pathname.startsWith('/services')) ? "Manage your jobs & earnings" :
    "Faster booking & real-time updates"
  );

  const resolvedAppIcon = appIcon || "/android-chrome-192x192.png";
        ...
        <>
            ...
            >
                <div className="bg-card border-2 border-primary/20 shadow-2xl rounded-2xl p-4 flex items-center gap-4 relative">
                    ...
                    <div className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 border relative overflow-hidden">
                        <Image src={resolvedAppIcon} alt="App Icon" width={40} height={40} className="rounded-lg shadow-sm object-cover aspect-square" />
                    </div>

                    <div className="flex-grow min-w-0">
                        <h4 className="text-sm font-bold text-foreground">Install {resolvedAppName}</h4>
                        <p className="text-[10px] text-muted-foreground line-clamp-1">{resolvedAppDesc}</p>
                    </div>
            ...
            >
                ...
                <button 
                    ...
                    className="absolute -top-2 -left-2 bg-destructive text-white rounded-full p-0.5 border-2 border-white shadow-md flex items-center justify-center w-4 h-4"
                >
                    <X size={10} />
                </button>
            </div>
                ...
                <DialogContent className="max-w-[90vw] rounded-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-left pt-2">
                            To install the app on your iPhone, follow these simple steps:
                        </DialogDescription>
                    </DialogHeader>
    ...
    >
      <div className="flex items-center w-full h-full p-2.5">
        ...
        <div className="ml-3 transition-all duration-500 opacity-0 group-hover:opacity-100 whitespace-nowrap">
          ...
          <p className="text-xs font-bold text-foreground">Install {resolvedAppName}</p>
        </div>
      </div>
